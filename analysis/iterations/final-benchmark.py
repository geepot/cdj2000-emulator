...
root = Path.cwd()
output = root / 'build/performance/final-comparison'
...
warm = {label: statistics.median(r['seconds'] for r in rows
        ...
        for label in ('baseline', 'final')}
report = dict(baseline_commit='c16d1cb', final_commit=subprocess.check_output(
...
Path('analysis/iterations/final-comparison.json').write_text(json.dumps(report, indent=2) + '\n')
