"""Run from the repository root; pre-direct-fetch is a git archive of e095f2b.
...
output = Path('build/performance/09-native')
...
for label, root in [('before', Path('build/performance/pre-direct-fetch')), ('after', ROOT)]:
...
report.update(medians=m, speedup=m['before'] / m['after'],
              ...
              baseline_commit='e095f2b', exact_trace_and_checkpoint_match=True,
              compiler=subprocess.check_output(['cc', '--version'], text=True),
              developer_dir=env.get('DEVELOPER_DIR'))
Path('analysis/iterations/09-native-direct-fetch.json').write_text(json.dumps(report, indent=2) + '\n')
