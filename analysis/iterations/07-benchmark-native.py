"""Run from the repository root; pre-coverage is a git archive of e048cc9.
...
output = Path('build/performance/07-native')
...
for label, root in [('before', Path('build/performance/pre-coverage')), ('after', ROOT)]:
...
report.update(medians=m, speedup=m['before'] / m['after'],
              ...
              baseline_commit='e048cc9', exact_trace_and_checkpoint_match=True,
              compiler=subprocess.check_output(['cc', '--version'], text=True),
              developer_dir=env.get('DEVELOPER_DIR'))
Path('analysis/iterations/07-native-coverage.json').write_text(json.dumps(report, indent=2) + '\n')
