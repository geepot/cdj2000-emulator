"""Run from the repository root; pre-fetch-scratch is a git archive of 7ff835e.
...
output = Path('build/performance/08-native')
...
for label, root in [('before', Path('build/performance/pre-fetch-scratch')), ('after', ROOT)]:
...
report = dict(steps=1000000, input_sha256=hashlib.sha256(checkpoint.read_bytes()).hexdigest(),
              trials=rows)
m = {label: statistics.median(r['seconds'] for r in rows if r['label'] == label)
     for label in ('before', 'after')}
report.update(medians=m, speedup=m['before'] / m['after'],
              time_reduction_percent=100 * (1 - m['after'] / m['before']),
              baseline_commit='7ff835e', exact_trace_and_checkpoint_match=True,
              compiler=subprocess.check_output(['cc', '--version'], text=True),
              developer_dir=env.get('DEVELOPER_DIR'))
Path('analysis/iterations/08-native-fetch-scratch.json').write_text(json.dumps(report, indent=2) + '\n')
