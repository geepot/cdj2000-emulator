...
Prepare build/performance/pre-direct-fetch with `git archive e095f2b`. Pin the same
...
root = Path.cwd()
output = root / 'build/performance/09-adjacent'
...
for trial in range(5):
    for label in (('baseline', 'final') if trial % 2 == 0 else ('final', 'baseline')):
        ...
        cwd = root / 'build/performance/pre-direct-fetch' if label == 'baseline' else root
...
warm = {label: statistics.median(r['seconds'] for r in rows
        ...
        for label in ('baseline', 'final')}
report = dict(baseline_commit='e095f2b', base_of_candidate_commit=subprocess.check_output(
    ...
    baseline_trace_mode='compact', final_trace_mode='compact',
...
Path('analysis/iterations/09-adjacent.json').write_text(json.dumps(report, indent=2) + '\n')
